...
平时用 WebUI 和「操作」按钮就够了。模块不往系统里装命令，需要时用完整路径：
`su -c sh /data/adb/modules/po0fw/po0fw.sh help`
...
| `INSECURE` / `PIN` | 0 / 空 | 服务端是自签证书时：`INSECURE=1` + `PIN=sha256//…`（WebUI「证书信息」可查） |
| `TIMEOUT` | 10 | 单次请求超时（秒） |
...
- `证书校验失败`：先在 WebUI 点「证书信息」。若确认是服务端自签证书，设 `INSECURE=1` 并填上显示的 `PIN`。
...
- 模块不挂载任何系统文件，KernelSU / APatch 不需要元模块。
